...
/** Typert boundary requires plain JSON: drop undefined fields instead of sending them. */
...
class SessionArchiveService extends TypertRemoteService {
  ...
  /**
   * Build the sessionId -> workspace record index, in workspaceIds display order.
   * Used by both `list()` (for workspace grouping) and `unarchive()` (to report
   * which workspace a restored session returns to). Returns a bare object, never
   * a live registry object.
   */
  workspaceIndex() {
    ...
    return { wsOrder, wsBySession }
  }

  /** Plain workspace summary owning this session, or undefined when ungrouped. */
  workspaceOf(id) {
    const record = this.workspaceIndex().wsBySession.get(asId(id))
  ...
  async list() {
    ...
    const { wsOrder, wsBySession } = this.workspaceIndex()
  ...
  async read(sessionId) {
    ...
    return jsonSafe({ id, title, docs: docs.slice(0, limit), truncated })
  }
  ...
  async delete(sessionId) {
    ...
    return jsonSafe({ ok: true, id, diskRemoved, warning })
  }

  /**
   * Unarchive one session: remove it from the registry-global archived set only.
   * DSH archiving never moves the `sessionIds` seat, so a restored session returns
   * to its original workspace and position automatically. Unlike `delete()`, this
   * never touches workspace accounting, the projection cache, or the disk log.
   */
  async unarchive(sessionId) {
    ...
    return jsonSafe({ ok: true, id, workspace: this.workspaceOf(id) })
  }
}
